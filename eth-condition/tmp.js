...
function parseAgreementId(runtime2, log) {
  try {
    if (!log.topics || log.topics.length < 2)
      return null;
    const idHex = bytesToHex(log.topics[1]);
    return BigInt(idHex);
  } catch (e) {
    runtime2.log(`parseAgreementId failed: ${e?.message ?? String(e)}`);
    return null;
  }
}
function makeIdenticalStringAggregation() {
  return {
    aggregate: (values) => {
...
var initWorkflow = (config) => {
  ...
  const onLog = (runtime2, log) => {
    ...
    const taskText = runtime2.runInNodeMode(fetchTaskText, makeIdenticalStringAggregation())().result();
    let task;
    try {
    ...
    const wr = evmClient.writeReport(runtime2, {
